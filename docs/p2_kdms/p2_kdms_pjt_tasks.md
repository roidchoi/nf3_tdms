...
### Phase 5: 성능 최적화 및 공백 백필 고도화

| ID | Task명 | 구현 범위 요약 | 상태 | 우선순위 | 의존성 | 시작일 | 완료일 |
|----|--------|--------------|------|---------|--------|--------|--------|
| T-010 | 수집 공백일 자동 감지 및 고효율 범위 백필 | 일봉/시총/분봉 수집에 대해 DB 적재 히스토리를 바탕으로 공백일을 자동 판별하여 단일 API 호출 범위 내로 병합 수집하도록 `daily_task.py` 리팩토링 | 완료 | High | T-008 | 2026-05-27 | 2026-05-27 |
...
| BACKFILL-OPT | 수집 공백 고효율 범위 백필 적용 | T-010 |
