...
> **작성일**: 2026-05-14
> **총 Task**: 9개 (Phase 1: 3개 / Phase 2: 3개 / Phase 3: 2개 / Phase 4: 1개)
...
**Phase 구분 기준:**
- Phase 1 (MVP): DB 인계 완료 + 일일 OHLCV·종목 마스터·시가총액 자동 수집 가동
- Phase 2 (수집 완성): 수정계수·재무제표·분봉 수집 + 전체 스케줄 자동화 완성
...
| ID | Task명 | 구현 범위 요약 | 상태 | 우선순위 | 의존성 | 시작일 | 완료일 |
|----|--------|--------------|------|---------|--------|--------|--------|
| T-004 | 수정계수 수집 + 역산 API (KIS) | `collectors/factor_calculator.py`(누적곱 알고리즘 검증 완료), `repositories/factor_repo.py`, `price_adjustment_factors` CRUD (`price_source` 컬럼으로 KIS/Kiwoom 구분), `/api/data/factors`, `/api/data/ohlcv/daily/adjusted`(조회 시 계산, 저장 금지) | 대기 | High | T-002 | - | - |
| T-005 | PIT 재무제표 수집 | `tasks/financial_task.py`(KIS `fetch_all_financial_data`), `repositories/financial_repo.py`, `financial_statements` / `financial_ratios` CRUD (PIT Key: `retrieved_at`), `financial_update(토 09:00)` 스케줄 등록 | 대기 | Medium | T-004 | - | - |
| T-006 | 분봉 수집 (Kiwoom) | `collectors/kiwoom_client.py`(KiwoomApiCore 래퍼 — 분봉 전용), `collectors/target_selector.py`(직전 분기 평균 거래대금 상위N), `tasks/backfill_task.py`(분봉 백필 + 시총 갭 복구 30일), `backfill_minute(토 10:20)` 스케줄 등록 | 대기 | Medium | T-003 | - | - |

### Phase 3: API 완성

| ID | Task명 | 구현 범위 요약 | 상태 | 우선순위 | 의존성 | 시작일 | 완료일 |
|----|--------|--------------|------|---------|--------|--------|--------|
| T-007 | 조회 API 완성 + Blacklist 패턴 | `/api/data/ohlcv/daily`, `/api/data/ohlcv/minute`, `/api/data/financials`, `/api/data/screening`, `/api/data/market-cap`, `/api/data/preview/{table}`, Blacklist 패턴(수집 실패 종목 사유 코드 관리, `daily_task.py` skip 로직 교체) | 대기 | Medium | T-005, T-006 | - | - |
...
| F-02 | 주가 수정계수 계산 및 관리 (KIS) | T-004 |
| F-03 | PIT 재무제표 수집 | T-005 |
| F-04 | 시가총액 수집 (pykrx) | T-003 |
| F-05 | 분봉 데이터 수집 (Kiwoom) | T-006 |
| F-06 | 종목 마스터 관리 | T-002 |
| API-데이터 | 데이터 조회 엔드포인트 9종 | T-002(`/stocks`), T-004(`/factors`, `/adjusted`), T-007(나머지 6종) |
| API-헬스 | 헬스·시스템 엔드포인트 9종 + WebSocket | T-003(`/tasks/{id}/run`), T-008(나머지) |
| SCHED | 3종 자동화 스케줄 | T-003(`daily_update`), T-005(`financial_update`), T-006(`backfill_minute`) |
| REFACTOR-1 | `db_manager.py` → `repositories/` 분리 | T-001(base), T-002(ohlcv, master), T-003(market_cap), T-004(factor), T-005(financial) |
| REFACTOR-2 | `kiwoom_rest.py` → `p1_shared/KiwoomApiCore` 통합 | T-006 |
