...
TDMS는 세 개의 독립적인 서브프로젝트로 구성된다.

```
nf3_tdms/
├── p1_kdms/       # 한국 시장 데이터 백엔드 (독립 실행 가능)
├── p2_usdms/      # 미국 시장 데이터 백엔드 (독립 실행 가능)
├── p3_manager/    # 통합 관리 UI + 공통 오케스트레이터
└── shared/        # 공통 모듈 (API 클라이언트, DB 유틸, 로거 등)
```

### 2.1 p1_kdms — 한국 시장 데이터 백엔드
...
| **독립성** | `p2_usdms`, `p3_manager` 없이도 단독 실행 가능 |
...
| **독립성** | `p1_kdms`, `p3_manager` 없이도 단독 실행 가능 |
...
### 2.3 p3_manager — 통합 관리 레이어

| 항목 | 내용 |
|---|---|
| **참조 원본** | KDMS frontend (Vue 3) + 두 시스템의 공통 관리 패턴 |
| **핵심 목표** | p1, p2를 모두 통합 모니터링·관리하는 단일 UI 및 오케스트레이터 |
| **독립성** | p1, p2가 실행 중일 때 연결; 단독으로는 의미 없음 |

**구현 범위**

- **통합 대시보드**: p1(KR), p2(US) 양쪽의 태스크 상태, 로그, 헬스 지표를 하나의 화면에서 표시
- **통합 스케줄 관리**: 두 시스템의 스케줄을 한 곳에서 조회·수정
- **통합 데이터 익스플로러**: KR/US 데이터를 시장별로 탭 전환하여 조회
- **통합 헬스 모니터링**: 데이터 신선도, 갭 탐지, 마일스톤을 시장별로 통합 표시
- **WebSocket 실시간 로그**: p1, p2 각각의 실행 로그를 탭으로 분리하여 실시간 스트리밍
...
| 시스템 | PIT Key | 적용 대상 |
|---|---|---|
| p1_kdms | `retrieved_at` (TIMESTAMPTZ) | financial_statements, financial_ratios |
| p2_usdms | `filed_dt` (DATE) | us_financial_facts, us_standard_financials, us_share_history |
...
|---|---|---|---|
| p1_kdms | `daily_ohlcv` | `price_adjustment_factors` | KIS/Kiwoom 이원화 관리 |
| p2_usdms | `us_daily_price` | `us_price_adjustment_factors` | Adj/Close 비율 저장 |
...
p1과 p2가 독립 실행 가능한 구조를 유지하면서도, 중복 구현을 피하기 위해 다음 컴포넌트는 `shared/` 공통 모듈로 관리한다.
...
| `KiwoomApiCore` | `kdms_origin/collectors/kiwoom_rest.py` | p1 전용이지만 shared에 배치하여 향후 확장성 확보 |
| `KisUsWrapper` | `usdms_origin/backend/collectors/kis_us_wrapper.py` | 미국 주식 KIS 래퍼 (p2 전용이지만 shared 배치) |
...
p1_kdms/collectors/kis_kr_client.py  → KisApiCore 상속, KR 전용 엔드포인트
p2_usdms/collectors/kis_us_client.py → KisApiCore 상속, US 전용 엔드포인트
```

> **핵심**: 토큰 캐시 파일을 공유하면 p1과 p2가 같은 KIS 계정을 사용할 때 불필요한 토큰 재발급을 방지하고 API 호출 제한에 여유를 확보한다.
...
python shared/ops/backup_manager.py --target all --tag pre_update

echo "[2/4] Verifying backup..."
python shared/ops/backup_manager.py --verify-last
...
python shared/ops/backup_manager.py --restore \
  --target kdms \
  --file backups/kdms/pre_update/checkpoint_20260428_030000.dump

# 복구 후 무결성 검증
python p1_kdms/ops/run_diagnostics.py
```
...
| 시간 (KST) | 대상 | 작업 |
|---|---|---|
| 17:10 (월~금) | p1_kdms | 일일 OHLCV + 팩터 + 시총 동기화 |
| 09:00 (토) | p1_kdms | PIT 재무 데이터 업데이트 |
| 10:20 (토) | p1_kdms | 분봉 백필 + 시총 갭 복구 |
| 07:00 (화~토) | p2_usdms | 일일 루틴 (Step 1~6: Master Sync → Market Data → Financial → Valuation) |
| 03:00 (매일) | shared | DB 자동 백업 (p1, p2 모두) |

> p1(KR 시장: 15:30 마감)과 p2(US 시장: 05:00 KST 마감)의 실행 시간대가 분리되어 서버 부하 충돌이 없다.
...
| 서브프로젝트 | PRD 문서 경로 | 상태 |
|---|---|---|
| p1_kdms | `docs/p1_kdms/p1_kdms_PRD.md` | ⬜ 미작성 |
| p2_usdms | `docs/p2_usdms/p2_usdms_PRD.md` | ⬜ 미작성 |
| p3_manager | `docs/p3_manager/p3_manager_PRD.md` | ⬜ 미작성 |
| shared | `docs/shared/shared_PRD.md` | ⬜ 미작성 |
...
### 10.1 가용성
- p1, p2 각각 **독립적으로 장애 격리** 가능. 한쪽 장애가 다른 쪽에 영향 없음.
- 일일 스케줄 작업 실패 시 다음 실행 주기에 자동 재시도 또는 갭 복구 수행.

### 10.2 확장성
- 향후 추가 시장(예: p4_jp_dms — 일본 시장)을 추가할 때 `shared/` 모듈을 재사용하고 동일 패턴으로 서브프로젝트를 확장할 수 있는 구조.
- `p3_manager`는 새로운 서브프로젝트 추가 시 설정 파일(YAML)에 엔드포인트를 등록하는 방식으로 확장 가능.

### 10.3 유지보수성
- 모든 하드코딩된 설정값은 `.env` 또는 YAML 설정 파일로 외부화.
- `shared/` 공통 모듈의 변경이 p1, p2의 동작을 의도치 않게 변경하지 않도록 **버전 태깅** 또는 인터페이스 분리 적용.
...
- 이상징후 감지 시 로그에 명확한 ERROR 레벨 기록 (p3_manager에서 집계 가능).
- WebSocket 실시간 로그는 p3_manager UI에서 확인.
...
| 번호 | 질문 | 결정 필요 주체 |
|---|---|---|
| Q1 | KIS API 계정이 p1/p2 공유인가, 별도인가? (토큰 캐시 공유 여부에 직접 영향) | 운영자 |
| Q2 | p3_manager가 p1/p2에 직접 DB 접근할 것인가, REST API만 사용할 것인가? | 아키텍처 결정 |
| Q3 | 분봉 데이터(minute_ohlcv)는 p1에서 계속 수집할 것인가? (스토리지 비용 vs 활용도) | 운영자 |
| Q4 | USDMS의 KIS US Wrapper는 계속 사용할 것인가, yfinance로 교체할 것인가? | p2 PRD에서 결정 |
