...
| T-007 | DB 기동 검증기 | `ops/startup_validator.py` — DB 접속 테스트, 핵심 테이블 존재·행수 검증, `check_volume_exists()` 연동, Hypertable 청크 상태 확인, `print_report()` 실패 항목별 구체적 조치 안내 출력, FastAPI lifespan 연동 패턴 검증, 단위 테스트 | 완료 | High | T-003, T-006 | - | - |
