# Task-005: 분봉 수집 (Kiwoom) 및 갭 복구 명세서

본 문서는 Kiwoom OpenAPI를 활용한 대상 종목의 분봉(1분봉) 데이터 수집 클라이언트 구현, 직전 분기 평균 거래대금 기준 Target 선정 모듈 개발, 그리고 백그라운드 분봉 백필 및 시가총액 데이터 갭 복구 파이프라인의 설계 및 테스트 명세를 다룹니다.

특히 사용자의 특별 지시사항에 따라, **시가총액 갭 복구 기간은 최근 30일이 아닌, 과거 데이터 적재일인 2025년 11월 이전부터 현재까지 전체 기간**을 조회하고 복구할 수 있도록 확장 설계합니다.
...
3. **Backfill Task Pipeline (`tasks/backfill_task.py`)**
   ...
   - **[특별 지시사항 반영]** `daily_market_cap` 테이블의 시가총액 누락일 보정 시, 검사 범위를 최근 30일로 고정하지 않고 **2025-11-01 이전부터 현재까지 전체 기간**으로 확장하여 pykrx 호출 복구 파이프라인 실행.
...
### 2) 시가총액 갭 복구 쿼리 (`daily_market_cap` 검증)
- 2025-11-01부터 현재일자 사이의 모든 개장일(`trading_calendar.opnd_yn = 'Y'`) 중, `daily_market_cap` 테이블에 데이터가 없는 날짜 리스트를 탐색하여 갭 복구.

---

## 3. 테스트 케이스 설계 (TDD 완료 기준)

구현 Agent는 다음 5가지 테스트 케이스를 우선 통과하도록 로직을 설계해야 합니다.
...
### 3) 사용자 특별 지시사항 (2025-11-01 이전 역사적 데이터 갭 복구 보장)
- **`test_market_cap_backfill_covers_from_november_2025`**
  - [검증]: `_backfill_market_cap_gaps` 실행 시 갭 복구의 탐색 시작일(`start_date`)이 반드시 2025-11-01 혹은 그 이전으로 잡혀 pykrx 수집 로직이 돌도록 구현되어 있는지 확인.
...
- 누락일 탐지, 분봉 백필, 시가총액 갭 복구(2025년 11월 이전부터 현재까지) 및 진척도 기입 파이프라인.
