...
| T-004 | 가치평가 + 재무비율 산출 | `engines/valuation_calculator.py`(`merge_asof(direction='backward')` PIT 매칭, PE/PB/PS/PCR/EV-EBITDA, TTM×4, 주식수 Hybrid Fallback, 50건 배치 저장), `engines/metric_calculator.py`(ROE/ROA/ROIC/GP-A/Debt Ratio/Current Ratio/Interest Coverage + YoY 성장률 Rev/Op/EPS), `repositories/valuation_repo.py`(`us_daily_valuation`, `us_financial_metrics` ON CONFLICT upsert) | 대기 | High | T-003 | - | - |
