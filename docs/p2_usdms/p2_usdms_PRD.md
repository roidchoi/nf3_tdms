...
p2_usdms는 USDMS 원본의 **백엔드 기능**을 정제·리팩토링하여 재구현한다.
UI는 `p3_manager`가 담당하며, p2는 데이터 수집·저장·API 제공에만 집중한다.

### 핵심 목표
1. 기존 운영 중인 `usdms_db` 데이터를 **단절 없이 인계**받아 수집 재개
2. SEC EDGAR 기반 CIK 중심 티커 관리 체계 유지
3. XBRL 재무 파싱 → PIT 표준화 파이프라인 보존
4. `p1_kdms`와 유사 기능의 구현 방향 통일 (DB 패턴, 운영 구조 등)
5. `p3_manager`가 REST API로 제어·조회할 수 있는 완결된 백엔드 제공
...
| 인계 전 백업 | `BackupManager --target usdms --tag pre_p2_migration` 실행 필수 |
...
- **p3_manager 연동**: `/api/admin/tasks/{task_id}/run`으로 수동 즉시 실행
...
| `kis_api_core.py` 중복 | kdms와 동일 코드 별도 존재 | `shared/KisApiCore`로 통합 (토큰 캐시 공유) |
| 운영 스크립트 산재 | `ops/`, `db_init/` 역할 혼재 | `ops/` = 일상 운영, `db_init/` = 초기화 전용으로 명확히 분리 |

### 5.3 p1_kdms와 통일할 구현 패턴

| 기능 | p1 방식 | p2 현재 | 통일 결과 |
|---|---|---|---|
| DB 커서 | `get_cursor()` context manager 도입 | 이미 `get_cursor()` 사용 | **p2 패턴 유지** (p1이 채택) |
| 운영 진입점 | `ops/` 폴더 집중 | `ops/` + `db_init/` 혼재 | **`ops/` 집중** (db_init은 초기화 전용) |
| 블랙리스트 | p2 패턴 도입 예정 | `BlacklistManager` 완비 | **p2 패턴이 기준** |
| 실시간 로그 | WebSocket (`/ws/logs`) | 미구현 | **p1 방식 도입** (p3_manager 연동) |
...
│   └── kis_us_client.py       # shared/KisApiCore → US 전용 래퍼
│
├── engines/                   # 계산 레이어
...
- [ ] `kis_api_core.py` → `shared/KisApiCore` 교체
...
*p3_manager 연동 인터페이스 상세는 `docs/p3_manager/p3_manager_PRD.md` 참조.*
